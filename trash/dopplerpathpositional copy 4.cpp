...
public:
    ...
    std::pair<Spectrum, Bool> sample(const Scene *scene,
        ...
        while (loop(active)) {
            ...
            if (dr::any_or<true>(active_em)) {
                ...
                result[active_em] = spec_fma(
                    throughput, dr::select(path2_occluded, 1.0, 0.5) * bsdf_val * em_weight * mis_em * length_weight, result);
            }
            ...
            SurfaceInteraction3f si_hit_sample = scene->ray_intersect(ray,
                                     /* ray_flags = */ +RayFlags::All,
                                     /* coherent = */ dr::eq(depth, 0u));



            Ray ray_temp = si.spawn_ray(dr::select(si_hit_sample.is_valid() && si.is_valid(), dr::normalize(si_hit_sample.p - si.p), si.to_world(bsdf_sample.wo))); //si.to_world(bsdf_sample.wo));//
            SurfaceInteraction3f si_hit_explicit = scene->ray_intersect(ray_temp,
                                     /* ray_flags = */ +RayFlags::All,
                                     /* coherent = */ dr::eq(depth, 0u));
            result = dr::norm(si_hit_sample.p - si_hit_explicit.p);
            result = dr::select(si_hit_sample.is_valid() && si.is_valid(), dr::norm(dr::normalize(si_hit_sample.p - si.p) - ray.d), 0);
            
            // result = si_hit_explicit.p;
            // result = dr::select(si_hit_sample.is_valid(), dr::normalize(si_hit_sample.p - si.p), ray.d);
            // result = si.to_world(bsdf_sample.wo);
            
            break;
            
            prev_si2 = si2;

            Mask use_positional_correlation = si.is_valid() && si2.is_valid();
            ...
            SurfaceInteraction3f si2_next = si;
            si2_next.time = ray.time;
            //dr::select(use_positional_correlation, si.adjust_time(ray2_time), si);

            // result = dr::select(use_positional_correlation, dr::norm(si.p - si2_next.p), 0);
            // break;
            
            ray2 = dr::select(use_positional_correlation, prev_si.spawn_ray(dr::normalize(si2_next.p - prev_si.p)), ray2);
            ray2.time = ray.time;
            
            //auto bsdf_val_2_anti = bsdf2->eval(bsdf_ctx2, prev_si2, prev_si2.to_local(ray2.d), use_positional_correlation);

            SurfaceInteraction3f si2_hit = scene->ray_intersect(ray2,
                                     /* ray_flags = */ +RayFlags::All,
                                     /* coherent = */ dr::eq(depth, 0u));

            // dr::select(use_positional_correlation, scene->ray_intersect(ray2,
            //                          /* ray_flags = */ +RayFlags::All,
            //                          /* coherent = */ dr::eq(depth, 0u)), si2_next);
            result = dr::select(use_positional_correlation, dr::select(((si2_hit.t < (dr::norm(si2_next.p - prev_si.p) - 1e-6)) || !si2_hit.is_valid() ), 0, dr::norm(si2_hit.p - si2_next.p)), 0);
            // result = dr::select(use_positional_correlation, dr::norm(si.p - si2_next.p), 0);
            active = false;
            break;


            // path2_occluded = (!si2.is_valid()) || (si2.t < (dr::norm(si2_next.p - prev_si2.p)));
            // throughput2 = dr::select(path2_occluded, 0.0, throughput2);
            // // si2.t = dr::norm(si2.p - prev_si2.p);
        ...
        return {
            /* spec  = */ result,//dr::select(valid_ray, result, 0.0),
            /* valid = */ valid_ray
        };
    }
