...
class TimeStratifiedSampler final : public PCG32Sampler<Float, Spectrum> {
public:
    MI_IMPORT_BASE(PCG32Sampler, m_sample_count, m_base_seed, m_rng, seed,
                   seeded, m_samples_per_wavefront, m_wavefront_size,
                   schedule_state, current_sample_index)
    MI_IMPORT_TYPES()

    TimeStratifiedSampler(const Properties &props) : Base(props) { }

    ref<Sampler<Float, Spectrum>> fork() override {
        TimeStratifiedSampler *sampler = new TimeStratifiedSampler(Properties());
    ...
    ref<Sampler<Float, Spectrum>> clone() override {
        return new TimeStratifiedSampler(*this);
    }
    ...
    Float next_1d_time(Mask active = true, int strategy = TIME_SAMPLING_UNIFORM) override {
        UInt32 p = current_sample_index();
        p = p % m_sample_count;
        Float j = m_rng.template next_float<Float>(active);
        Float r = (p + j) / m_sample_count;
        return r;
    }
    ...
    std::string to_string() const override {
        ...
        oss << "TimeStratifiedSampler[" << std::endl
...
private:
    TimeStratifiedSampler(const TimeStratifiedSampler &sampler) : Base(sampler) {}
};

MI_IMPLEMENT_CLASS_VARIANT(TimeStratifiedSampler, Sampler)
MI_EXPORT_PLUGIN(TimeStratifiedSampler, "Independent Sampler");
