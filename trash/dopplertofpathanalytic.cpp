...
public:
    ...
    Float eval_modulation_function_value_low_pass(Float _t, uint32_t function_type) const{
        Float t = dr::fmod(_t, 2 * M_PI);
        switch(function_type){
            case WAVE_TYPE_SINUSOIDAL: return dr::cos(t);
    ...
    Float eval_modulation_weight(Float ray_time, Float path_length) const
    {
        ...
        if(m_low_frequency_component_only){
            ...
            Float fg_t = 0.25 * eval_modulation_function_value_low_pass(t, m_sensor_modulation_function_type);
            return fg_t;
        }
