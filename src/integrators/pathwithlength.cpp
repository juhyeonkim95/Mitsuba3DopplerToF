...
public:
    ...
    PathLengthIntegrator(const Properties &props) : Base(props) {
        m_aov_names.push_back("path_length_2");
        m_aov_names.push_back("path_length_3");
        m_aov_names.push_back("path_length_4");
        m_aov_names.push_back("radiance_2");
        m_aov_names.push_back("radiance_3");
        m_aov_names.push_back("radiance_4");
        //for(int i=0; i<m_max_depth - 1; i++){
        //    m_aov_names.push_back("path_length_" + std::to_string(i));
        //}
    }
    ...
    std::pair<Spectrum, Bool> sample(const Scene *scene,
        ...
        Float eta                     = 1.f;
        UInt32 depth                  = 0;
        Float path_length_2             = 0;
        Float path_length_3             = 0;
        Float path_length_4             = 0;
        Spectrum result_2               = 0.f;
        Spectrum result_3               = 0.f;
        Spectrum result_4               = 0.f;
        ...
        dr::Loop<Bool> loop("Path Length Tracer", sampler, ray, throughput, result,
        ...
        while (loop(active)) {
            ...
            path_length += dr::select(si.is_valid(), si.t, 0.f); 
            ...
            if (dr::any_or<true>(active_em)) {
                ...
                path_length_2 += dr::select(depth == 0, dr::select(si.is_valid(), ds.dist, 0.f), path_length_2);
                path_length_3 += dr::select(depth == 1, dr::select(si.is_valid(), ds.dist, 0.f), path_length_3);
                path_length_4 += dr::select(depth == 2, dr::select(si.is_valid(), ds.dist, 0.f), path_length_4);
...
MI_IMPLEMENT_CLASS_VARIANT(PathLengthIntegrator, MonteCarloIntegrator)
MI_EXPORT_PLUGIN(PathLengthIntegrator, "Path Length Tracer integrator");
